...
let sb=null, session=null, me=null, profiles=[], entries=[], reactions=[], rewardChoices=[], challengePool=[], proposals=[], proposalVotes=[], ratings=[], groupAssignments=[], dailyAssignments=[], dailyCompletions=[], achievements=[], currentView='home', pendingProof=null, pendingAvatar=null, signedCache={};
...
async function loadData(){
 let [p,e,r,w,reward,cp,pr,pv,cr,ga,da,dc,ach]=await Promise.all([
  ...
  sb.from('achievements').select('*').order('achieved_on',{ascending:false})
 ]);
 ...
 dailyAssignments=da.data||[];dailyCompletions=dc.data||[];achievements=ach.data||[];
 await ensureAssignments();await syncAchievements();
}
function startRealtime(){
 stopRealtime();
 ['entries','reactions','profiles','weekly_challenges','reward_choices','challenge_pool','challenge_proposals','challenge_proposal_votes','challenge_ratings','group_challenge_assignments','daily_challenge_assignments','daily_challenge_completions','achievements'].forEach(table=>{
...
function hallOfFame(mk){let rows=allApprovedUsers().map(p=>({p,d:monthlyReviewData(p.id,mk),streak:streak(p.id),outdoor:entries.filter(e=>e.user_id===p.id&&e.entry_date.startsWith(mk)&&e.kind==='activity'&&['walk','hike'].includes(e.activity)).length,daily:dailyCompletions.filter(x=>x.user_id===p.id&&x.challenge_date.startsWith(mk)).length}));if(!rows.length)return{};let max=fn=>rows.slice().sort((a,b)=>fn(b)-fn(a))[0];return {champ:max(x=>x.d.pts),steps:max(x=>x.d.steps),streak:max(x=>x.streak),healthy:max(x=>x.d.foodDays),outdoor:max(x=>x.outdoor),social:max(x=>x.daily)}}
...
function challengePoolHTML(){return ['weekly','group','daily'].map(type=>{let title=type==='weekly'?'🎯 Wochenchallenges':type==='group'?'👥 Gruppen-Challenges':'☀️ Tageschallenges',list=challengePool.filter(c=>c.challenge_type===type);return `<h3>${title}</h3><div class="grid grid2">${list.map(c=>{let r=ratings.filter(x=>x.challenge_pool_id===c.id),creator=profileById(c.created_by);return `<button class="choice ${!poolAvailable(c)?'disabled':''}" onclick="openChallengeDetail('${c.id}')"><div><b>${escapeHtml(c.emoji)} ${escapeHtml(c.name)}</b>${!poolAvailable(c)?' 🔒':''}</div><div class="tiny muted">${escapeHtml(c.description)}</div><div class="tiny muted" style="margin-top:5px">+${c.points} P · 👍 ${r.filter(x=>x.rating==='again').length} · 😐 ${r.filter(x=>x.rating==='okay').length} · 👎 ${r.filter(x=>x.rating==='never').length}${creator?` · von ${escapeHtml(firstName(creator))}`:''}</div></button>`}).join('')}</div>`}).join('')}
...
function hallOfFameHTML(){let h=hallOfFame(monthKey());if(!h.champ)return '<div class="card pad muted">Noch keine Monatsdaten.</div>';let row=(em,title,x)=>`<div class="rankRow"><span>${em}</span><b>${title}</b><b>${escapeHtml(firstName(x?.p))}</b></div>`;return `<div class="card pad">${row('👑','Monatschampion',h.champ)}${row('👟','Schrittmonster',h.steps)}${row('🔥','Streak-Master',h.streak)}${row('🥗','Healthy Hero',h.healthy)}${row('🌤️','Outdoor-König',h.outdoor)}${row('❤️','Social Hero',h.social)}</div>`}
